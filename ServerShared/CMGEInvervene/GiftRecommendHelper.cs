...
namespace ServerShared
{
    ...
    public class RecommendReturn
    {
        ...
        public class RecommendGiftInfo
        {
    ...
    public class RunAwayReturnInfo
    {
    ...
    public class GiftRecommendHelper
    {
        ...
        private static async Task<RecommendGiftInfo> GetRecommendGiftInfo(Dictionary<string, object> data)
        {
            ...
            try
            {
                ...
                //modify by joker .so that when have no internet .always can run 
                info.Data_Box = msg==null ? "": msg.data == null ? "" : msg.data.data_box;
                //info.Data_Box = msg.data == null ? "" : msg.data.data_box;
                if(msg == null)
                {
                    Log.Info((object)"got recommend gift fail : had not connect internet");
                    return info;
                }
                if (msg.code != 0)
                {
        ...
        public static async Task SyncSdkInterveneInfo()
        {
            DataList dataList = DataListManager.inst.GetDataList("RunAwayActivityType");
            foreach (var kv in dataList)
            {
                ...
                Dictionary<string, object> data = new Dictionary<string, object>
                {
                    ...
                    {"game_ids", "6360,6361,6362,7307"},
                    {"business_type", kv.Value.GetInt("BusinessType")},
                    {"intervene_id", kv.Value.GetString("InterveneId")},
                    {"intervene_name", kv.Value.GetString("InterveneName")},
                    {"remark", kv.Value.GetString("Remark")},
                    {"state", 1},
                    {"create_time", 1},
                    {"update_time", time},
                    {"timestamp", time},
                ...
                try
                {
                    ...
                    Log.Info($"SyncSdkInterveneInfo response {answer}");
                }
                catch (Exception ex)
                {
                    Log.Info(answer);
        ...
        public static async Task<InterveneResultInfo> GetRunAwayInfo(string account, string serverId, string roleId, long gameId)
        {
            long uUid = 0L;
            long.TryParse(account, out uUid);
            Dictionary<string, object> data = new Dictionary<string, object>
            {
                {"task_type", TASK_TYPE_RUN_AWAY},
                {"game_base_id", game_id},
                {"game_id", gameId},
                {"cp_uid", uUid},
            ...
            try
            {
                ...
                if(msg == null)
                {
                    Log.Info((object)"got recommend gift fail :not connect internet");
                    return null;
                }
