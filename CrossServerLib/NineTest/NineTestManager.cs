...
namespace CrossServerLib
{
    public class NineTestManager
    {
        ...
        public NineTestManager(CrossServerApi server)
        {
            this.server = server;
            UpdatePeriod();
        }
        ...
        public void UpdatePeriod()
        {
            ...
            if (RechargeLibrary.InitRechargeGiftTime(RechargeGiftType.NineTest, server.Now(), out model))
            {
        ...
        {
            NineTestRank rank = server.RankMng.GetNineTestRank(groupId);
            if (rank == null)
            {
                server.RankMng.AddNineTestRank(groupId);
                rank = server.RankMng.GetNineTestRank(groupId);
            }
        ...
        public void Clear()
        {
            server.RankMng.ClearNineTestRank();
            UpdatePeriod();
        }
                ...
                if (rank != null)
                {
                    rank.LoadInitRankFromRedis(() =>
                    {
                        ...
                        foreach (var rankItem in uidRankInfoDic)
                        {
                            ...
                            {
                                RankRewardInfo rewardInfo = NineTestLibrary.GetRankRewardInfo(period, rankItem.Value.Rank);
                                if (rewardInfo == null)
                                {
                                    break;
                                }
                                else
                                {
                                    ...
                                    server.TrackingLoggerMng.TrackRankEmailLog(groupId, 0, RankType.NineTest.ToString(), rankItem.Value.Uid, rankItem.Value.Score, rewardInfo.EmailId, rankItem.Value.Rank, server.Now());
                        ...
                        server.RelationManager.SendRankInfoToRelation("nine_test", rankInfoList, randMainId);
                    });
                }
            }
        }
