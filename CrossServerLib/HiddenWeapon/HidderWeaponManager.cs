...
{
    public class HidderWeaponManager
    {
        private CrossServerApi server { get; set; }

        private HidderWeaponConfig config { get; set; }
        private Dictionary<int, int> currentValues = new Dictionary<int, int>();
        public HidderWeaponManager(CrossServerApi server)
        {
            this.server = server;

            LoadHidderWeaponValueFromRedis();

            config = HidderWeaponLibrary.GetHidderWeaponConfig(2);
        }

        protected void LoadHidderWeaponValueFromRedis()
        {
            OperateGetHidderWeaponValue op = new OperateGetHidderWeaponValue();
        ...
        {
            HidderWeaponRank rank = server.RankMng.GetHidderWeaponRank(groupId);
            if (rank != null)
            {
                //获取rank
        ...
        public bool UpdatePlayerValue(int groupId, int uid, int value)
        {
            ...
            if (rank == null)
            {
                server.RankMng.AddHidderWeaponRank(groupId);
                //获取rank
                rank = server.RankMng.GetHidderWeaponRank(groupId);
            }
            else
            {
            ...
            else if (totalValue + value >= config.MaxValue)
            {
                ...
                announcementMsg.Type = (int)ANNOUNCEMENT_TYPE.HIDDER_WEAPON_MAX_VALUE;

                OperateGetCrossRankScore op = new OperateGetCrossRankScore(RankType.HidderWeapon, groupId, 0, 0, 1);
                server.CrossRedis.Call(op, ret =>
                {
                    MSG_CorssR_RECORD_RANK_ACTIVE_INFO rankActiveMsg = new MSG_CorssR_RECORD_RANK_ACTIVE_INFO();
                    rankActiveMsg.RankType = "hidderWeapon";
                        ...
                        if (firstModel != null)
                        {
                            ...
                            if (fitstPlayerInfo != null)
                            {
                                ...
                                if (mainId > 0)
                                {
                                    ...
                                    server.TrackingLoggerMng.TrackRankEmailLog(groupId, 0, RankType.HidderWeapon.ToString(), firstModel.Uid, firstModel.Score, config.FirstEmail, 1, server.Now());
                                    //BI
                                    server.KomoeEventLogRankFlow(firstModel.Uid, mainId, RankType.HidderWeapon, 1, 1, firstModel.Score, RewardManager.GetRewardDic(config.FirstReward));
                                }
                    ...
                    if (playerModel != null)
                    {
                        ...
                        if (playerInfo != null)
                        {
                            ...
                            if (mainId > 0)
                            {
                                ...
                                server.TrackingLoggerMng.RecordSendEmailRewardLog(msg.Uid, msg.EmailId, msg.Reward, msg.Param, msg.MainId, server.Now());
                                server.TrackingLoggerMng.TrackRankEmailLog(groupId, 0, RankType.HidderWeapon.ToString(), playerModel.Uid, playerModel.Score, config.luckilyEmail, 0, server.Now());
                                //BI
                                server.KomoeEventLogRankFlow(playerModel.Uid, mainId, RankType.HidderWeapon, playerModel.Rank, playerModel.Rank, playerModel.Score, RewardManager.GetRewardDic(config.luckilyReward));
                            }
                            announcementMsg.List.Add(mainId.ToString());
                            announcementMsg.List.Add(name);
        ...
        private void ChangeTotalValue(int groupId, int value)
        {
            ...
            server.CrossRedis.Call(new OperateSetHidderWeaponValue(groupId, value));
        }

        public void Clear()
        {
            currentValues.Clear();
            server.CrossRedis.Call(new OperateDeleteHidderWeaponValue());
            server.RankMng.ClearHidderWeaponRank();
            server.NotesMng.Clear(NotesType.HidderWeaponItem);

            MSG_CorssR_CLEAR_VALUE msg = new MSG_CorssR_CLEAR_VALUE();
            msg.GiftType = (int)RechargeGiftType.HiddenWeapon;
