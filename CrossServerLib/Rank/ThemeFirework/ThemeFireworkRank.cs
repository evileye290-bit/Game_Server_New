...
{
    public class ThemeFireworkRank : BaseRank
    {
        public ThemeFireworkRank(CrossServerApi server, RankType rankType = RankType.ThemeFirework) : base(server, rankType)
        {
        }

        public void UpdateScore(int uid, int value)
        {
            server.CrossRedis.Call(new OperateUpdateCrossRankScore(RankType.ThemeFirework, groupId, paramId, uid, value, server.Now()));
