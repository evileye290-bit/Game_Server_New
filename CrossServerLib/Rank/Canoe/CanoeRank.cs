...
{
    public class CanoeRank : BaseRank
    {
        public CanoeRank(CrossServerApi server, RankType rankType = RankType.Canoe) : base(server, rankType)
        {
        }

        public void UpdateScore(int uid, int value)
        {
            server.CrossRedis.Call(new OperateUpdateCrossRankScore(RankType.Canoe, groupId, paramId, uid, value, server.Now()));
