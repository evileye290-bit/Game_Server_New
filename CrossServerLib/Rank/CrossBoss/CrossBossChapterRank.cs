...
{
    public class CrossBossChapterRank : BaseRank
    {
        public CrossBossChapterRank(CrossServerApi server, RankType rankType = RankType.CrossBoss) : base(server, rankType)
        {
        }
        ...
        public void SetScore(int uid, int value)
        {
            RankBaseModel ownerInfo = GetRankBaseInfo(uid);
            if (ownerInfo != null)
            {
            ...
            server.CrossRedis.Call(new OperateUpdateCrossRankScore(rankType, groupId, paramId, uid, value, server.Now()));
        }
