...
{
    public class CarnivalBossRank : BaseRank
    {
        public CarnivalBossRank(CrossServerApi server, RankType rankType = RankType.CarnivalBoss) : base(server, rankType)
        {
        }     

        public void UpdateScore(int uid, int value)
        {
            server.CrossRedis.Call(new OperateUpdateCrossRankScore(RankType.CarnivalBoss, groupId, paramId, uid, value, server.Now()));
