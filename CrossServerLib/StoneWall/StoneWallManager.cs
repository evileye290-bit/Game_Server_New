...
namespace CrossServerLib
{
    public class StoneWallManager
    {
        ...
        private StoneWallConfig config { get; set; }
        private Dictionary<int, int> currentValues = new Dictionary<int, int>();

        public StoneWallManager(CrossServerApi server)
        {
            this.server = server;

            LoadStoneWallValueFromRedis();

            config = StoneWallLibrary.GetStoneWallConfig(2);
        }

        protected void LoadStoneWallValueFromRedis()
        {
            OperateGetStoneWallValue op = new OperateGetStoneWallValue();
        ...
        {
            StoneWallRank rank = server.RankMng.GetStoneWallRank(groupId);
            if (rank != null)
            {
                //获取rank
        ...
        public bool UpdatePlayerValue(int groupId, int uid, int value)
        {
            ...
            if (rank == null)
            {
                server.RankMng.AddStoneWallRank(groupId);
                //获取rank
                rank = server.RankMng.GetStoneWallRank(groupId);
            }
            else
            {
            ...
            else if (totalValue + value >= config.MaxValue)
            {
                ...
                announcementMsg.Type = (int)ANNOUNCEMENT_TYPE.STONE_WALL_MAX_VALUE;

                OperateGetCrossRankScore op = new OperateGetCrossRankScore(RankType.StoneWall, groupId, 0, 0, 1);
                server.CrossRedis.Call(op, ret =>
                {
                    MSG_CorssR_RECORD_RANK_ACTIVE_INFO rankActiveMsg = new MSG_CorssR_RECORD_RANK_ACTIVE_INFO();
                    rankActiveMsg.RankType = "stoneWall";
                        ...
                        if (firstModel != null)
                        {
                            ...
                            if (fitstPlayerInfo != null)
                            {
                                ...
                                if (mainId > 0)
                                {
                                    ...
                                    server.TrackingLoggerMng.TrackRankEmailLog(groupId, 0, RankType.StoneWall.ToString(), firstModel.Uid, firstModel.Score, config.FirstEmail, 1, server.Now());

                                    //BI
                                    server.KomoeEventLogRankFlow(firstModel.Uid, mainId, RankType.StoneWall, 1, 1, firstModel.Score, RewardManager.GetRewardDic(config.FirstReward));
                                }
                    ...
                    if (playerModel != null)
                    {
                        ...
                        if (playerInfo != null)
                        {
                            ...
                            if (mainId > 0)
                            {
                                ...
                                server.RelationManager.WriteToRelation(msg, mainId);
                                server.TrackingLoggerMng.TrackRankEmailLog(groupId, 0, RankType.StoneWall.ToString(), playerModel.Uid, playerModel.Score, config.luckilyEmail, 0, server.Now());
                                server.TrackingLoggerMng.RecordSendEmailRewardLog(msg.Uid, msg.EmailId, msg.Reward, msg.Param, msg.MainId, server.Now());
                                //BI
                                server.KomoeEventLogRankFlow(playerModel.Uid, mainId, RankType.StoneWall, playerModel.Rank, playerModel.Rank, playerModel.Score, RewardManager.GetRewardDic(config.luckilyReward));
                            }
                            announcementMsg.List.Add(mainId.ToString());
                            announcementMsg.List.Add(name);
        ...
        private void ChangeTotalValue(int groupId, int value)
        {
            ...
            server.CrossRedis.Call(new OperateSetStoneWallValue(groupId, value));
        }

        public void Clear()
        {
            currentValues.Clear();
            server.CrossRedis.Call(new OperateDeleteStoneWallValue());
            server.RankMng.ClearStoneWallRank();
            server.NotesMng.Clear(NotesType.StoneWall);

            MSG_CorssR_CLEAR_VALUE msg = new MSG_CorssR_CLEAR_VALUE();
            msg.GiftType = (int)RechargeGiftType.StoneWall;
