...
{
    class FsmPetAttackState : FsmBaseState
    {
        public FsmPetAttackState(FieldObject owner) : base(owner)
        {
            fsmStateType = FsmStateType.PET_ATTACK;
        }
        ...
        protected override void Start(FsmStateType prevState)
        {
            ...
            chasing = false;
            if (!pet.SkillEngine.TryFetchOneSkill(out skill, out target, out targetPos))
            {
                // 没找到合适技能，重新巡逻
            ...
            if (InSkillRange())
            {
                skill.InitCastParam(targetPos - pet.Position, targetPos, target == null ? 0 : target.InstanceId);
        ...
        private void ToSurroundOrSkill()
        {
            ...
            if (target != null)
            {
                ...
                {
                    Vec2 toPos = pet.Position;
                    nextParam = Tuple.Create(toPos, target, skill);
                    GoToNextState(FsmStateType.PET_SURROUND);
                }
                else
                {
                    GoToNextState(FsmStateType.SKILL);
        ...
        private void StartChasing()
        {
            ...
            Vec2 temp = targetPos - pet.Position;
            float distance = (float)temp.GetLength();
            float needLength = length - distance;
            if (needLength > 0)
            {
                dest = pet.Position + temp * (needLength + 0.1f) / distance;
            }
            else
            {
        ...
        private bool InSkillRange()
        {
            float targetRadius = target == null ? 0f : target.Radius;
            float casterRadius = owner.Radius;
            return Vec2.GetRangePower(pet.Position, targetPos) <=
