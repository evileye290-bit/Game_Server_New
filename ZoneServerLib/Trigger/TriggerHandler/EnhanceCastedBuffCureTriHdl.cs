...
{
    public class EnhanceCastedBuffCureTriHdl : BaseTriHdl
    {
        private readonly int buffId = 0;
        private readonly int ratio;
        public EnhanceCastedBuffCureTriHdl(BaseTrigger trigger, TriggerHandlerType handlerType, string handlerParam)
            : base(trigger, handlerType, handlerParam)
        {
            string[] paramArr = handlerParam.Split(':');
            if (paramArr.Length != 2)
            {
                Log.Warn("init enhance casted buff cure tri hdl failed, invalid handler param {0}", handlerParam);
                return;
            }
        ...
        public override void Handle()
        {
            ...
            buff.SetParamN((int)(buff.N * (1 + ratio * 0.0001f)));
#if DEBUG
