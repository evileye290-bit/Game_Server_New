...
{
    public class CarnivalBossDungeon : DungeonMap
    {        
        ...
        public CarnivalBossDungeon(ZoneServerApi server, int mapId, int channel) : base(server, mapId, channel)
        {
        }
        ...
        public override void CreateHero(Hero hero, bool add2Aoi = true)
        {
            ...
            int pcCount = hero.HeroInfo.CarnivalBossQueueNum <= 1 ? AttackerPosIndex : DefenderPosIndex;

            PlayerChar owner = hero.Owner as PlayerChar;

            int pos = hero.HeroInfo.CarnivalBossPositionNum;
        ...
        protected override void Start()
        {
            ...
            lastDegree = player.CarnivalBossMng.Info.Degree;
            degree = 0.0;
        }
        ...
        protected override void Failed()
        {
            ...
            foreach (var kv in PcList)
            {
                ...
                if (!isQuitDungeon)
                {
                    player.UpdateCounter(CounterType.CarnivalBossCount, 1);
                    player.AddCarnivalBossDegree(degree);
                }
                int pointState = 2;
                if (isQuitDungeon)
                {
        ...
        protected override void Success()
        {
            ...
            foreach (var kv in PcList)
            {
                try
                {
                    ...
                    player.AddRewards(mng, ObtainWay.CarnivalBoss);
                    ...
                    player.UpdateCounter(CounterType.CarnivalBossCount, 1);
                    player.AddCarnivalBossDegree(degree, killed);
