...
{
    public class CrossChallengeGuessingManager
    {
        ...
        public CrossChallengeGuessingManager(RelationServerApi server)
        {
            this.server = server;
        ...
        {
            //加载前8
            OperateGetCrossChallengeGuessingInfo op = new OperateGetCrossChallengeGuessingInfo(server.MainId, timingGuessingList.Keys.ToList());
        ...
        {
            //加载前8
            OperateGetCrossChallengeGuessingChooseInfo op = new OperateGetCrossChallengeGuessingChooseInfo(server.MainId, timingGuessingList);
            server.GameRedis.Call(op, ret =>
            {
        ...
        {
            //加载前8
            OperateGetCrossChallengeGuessingRewardInfo op = new OperateGetCrossChallengeGuessingRewardInfo(server.MainId, timingGuessingList);
            server.GameRedis.Call(op, ret =>
            {
                timingGuessingList = op.timingGuessingList;
            });
        ...
        public void ClearGuessingInfo()
        {
            TeamId = 0;
            InitGuessingList();
            server.GameRedis.Call(new OperateClearCrossChallengeGuessingInfo(server.MainId, timingGuessingList.Keys.ToList()));
        }
        ...
        public void GetGuessingPlayersInfo(int uid)
        {
            ...
            MSG_RC_GET_CROSS_CHALLENGE_GUESSING_INFO msg = new MSG_RC_GET_CROSS_CHALLENGE_GUESSING_INFO();
            if (uids.Count > 0)
            {
        ...
        public void SetGuessingPlayers(int timingId, int uid1, int uid2, int teamId)
        {
            ...
            manager.Player2 = uid2;
            manager.TeamId = teamId;
            server.GameRedis.Call(new OperateSetCrossChallengeGuessingInfo(server.MainId, timingId, uid1, uid2, teamId));
        ...
        public void SetPlayerChoose(int timingId, int uid, int choose)
        {
            ...
            server.GameRedis.Call(new OperateSetCrossChallengeGuessingChoose(server.MainId, timingId, uid, choose));
        }
        ...
        public void SetPlayerReward(int timingId, int uid, string reward)
        {
            ...
            server.GameRedis.Call(new OperateSetCrossChallengeGuessingReward(server.MainId, timingId, uid, reward));
        }
        ...
        public int CheckGuessingModel(int timingId)
        {
            ...
            CrossGuessingManagerModel manager = GetGuessingItem(timing);
            if (manager == null)
            {
                switch (timing)
                {
                    ...
                    case CrossBattleTiming.BattleTime6:
                        errorCode = (int) ErrorCode.CrossChallengeGuessingNotOpen;
                        break;
                    default:
        ...
        public void SendGuessingReward(int timingId, RepeatedField<int> uids)
        {
            ...
            if (manager != null)
            {
                int emailId = CrossChallengeLibrary.GuessingEmail;
